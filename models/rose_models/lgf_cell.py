...
from .utils import (
    ...
    calculate_residual_random_walk_matrix,
    calculate_reverse_random_walk_matrix,
...
class LGFCell(LightningModule):
    ...
    def __init__(self, adj_mx, args):
        ...
        if self.filter_type == "laplacian":
            supports.append(calculate_scaled_laplacian(adj_mx, lambda_max=None, undirected=False))
            ...
            supports.append(calculate_residual_random_walk_matrix(adj_mx).T)
        ...
        if self.use_gc_ru:
            ...
            self.ru_weights = nn.Parameter(
                torch.empty(
                    (input_size * num_matrices, 2 * self.rnn_units),
                )
            )
            # device=device)) TODO
            self.ru_biases = nn.Parameter(torch.zeros((2 * self.rnn_units)))  # , device=device)) TODO
        else:  # fc
            self.ru_weights = nn.Parameter(torch.empty((input_size, 2 * self.rnn_units)))
            self.ru_biases = nn.Parameter(torch.zeros((2 * self.rnn_units)))  # , device=device)) TODO
        if self.use_gc_c:
            # gconv
            self.c_weights = nn.Parameter(torch.empty((input_size * num_matrices, self.rnn_units)))
            self.c_biases = nn.Parameter(torch.zeros((self.rnn_units)))  # , device=device)) TODO
        else:
            # fc
    ...
    def _build_sparse_matrix(cls, mat):
        ...
        mat = torch.sparse_coo_tensor(indices.T, mat.data, mat.shape)  # , device=device) TODO
        return mat
    ...
    def forward(self, inputs, hx):
        ...
        r, u = torch.split(tensor=value, split_size_or_sections=self.rnn_units, dim=-1)
        del value
        ...
        c = c_opt(inputs, r * hx, self.rnn_units, is_c_opt=True)  # element-wise multiplication
    ...
    def _gconv(self, inputs, state, output_size, is_c_opt=False):
        ...
        """
        batch_size = inputs.shape[0]  # self.batch_size TODO bring to Rose attention
        ...
        num_matrices = len(self.supports) * self.max_diffusion_step + 1  # Adds for x itself.
    ...
    def _fc(self, inputs, state, output_size, is_c_opt=False):
        ...
        """
        batch_size = inputs.shape[0]  # self.batch_size
        num_nodes = self.num_nodes
