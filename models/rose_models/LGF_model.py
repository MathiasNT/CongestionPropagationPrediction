    ...
    def forward(self, inputs, hidden_state=None):
        ...
        if hidden_state is None:
            hidden_state = torch.zeros(
                (self.num_rnn_layers, inputs.shape[0], self.num_nodes, self.rnn_units), device=self.device
            )
        hidden_states = []
...
class SimpleGNN(BaseModelClass, Seq2SeqAttrs):
    ...
    def __init__(self, adj_mx, args, learning_rate, config, pos_weights):
...
class InformedGNN_v2(BaseModelClass, Seq2SeqAttrs):
    ...
    def __init__(self, adj_mx, args, learning_rate, config, pos_weights):
        ...
        args.use_gc_ru = False  # TODO fix this hack
        self.fc_net_info = nn.Linear(
            in_features=config["network_in_size"] + config["info_in_size"], out_features=config["rnn_hidden_size"]
        )
        args.input_dim = config["rnn_hidden_size"]
        self.incident_encoder = Encoder(self.adj_mx, args)
        self.fc_incident = nn.Linear(in_features=config["rnn_hidden_size"], out_features=config["fc_hidden_size"])
    ...
    def forward(self, inputs, incident_info, network_info):
        ...
        inputs = inputs.permute(2, 0, 1, 3)
        seq_len, batch_size, num_nodes, _ = inputs.shape
        encoder_hidden_state = None
        for t in range(self.input_len):
            next_hidden_state, encoder_hidden_state = self.timeseries_encoder(inputs[t], encoder_hidden_state)
        ...
        info_embed = self.fc_net_info(torch.cat([padded_info, network_info], dim=-1))
...
class InformedGNN_v3(BaseModelClass, Seq2SeqAttrs):
    """Lightning module for Latent Graph Forecaster model.
    Attributes:
      adj_mx: initialize if learning the graph, load the graph if known
      encoder: encoder module
    ...
    def __init__(self, adj_mx, args, learning_rate, config, pos_weights):
        ...
        self.mlp_shared = nn.Sequential(
            nn.Linear(in_features=config["fc_hidden_size"] * 2, out_features=config["fc_hidden_size"]),
            self.activation,
            nn.Linear(in_features=config["fc_hidden_size"], out_features=config["fc_hidden_size"]),
        )
        self.fc_classifier = torch.nn.Linear(in_features=config["fc_hidden_size"], out_features=1)
    ...
    def forward(self, inputs, incident_info, network_info):
        ...
        hn_fc = self.mlp_shared(torch.cat([timeseries_hn_fc, incident_hn_fc], dim=-1))
...
class InformedGNN_v4(BaseModelClass, Seq2SeqAttrs):
    """Lightning module for Latent Graph Forecaster model.
    Attributes:
      adj_mx: initialize if learning the graph, load the graph if known
      encoder: encoder module
    ...
    def __init__(self, adj_mx, args, learning_rate, config, pos_weights):
        ...
        args.input_dim = config["rnn_hidden_size"] * 2
        self.incident_encoder = Encoder(self.adj_mx, args)
        self.fc_incident = nn.Linear(in_features=config["rnn_hidden_size"], out_features=config["fc_hidden_size"])
    ...
    def forward(self, inputs, incident_info, network_info):
        ...
        info_embed = self.mlp_net_info(torch.cat([padded_info, network_info], dim=-1))
        info_embed = torch.relu(info_embed)
        incident_hidden_state, _ = self.incident_encoder(
            torch.cat([info_embed, timeseries_hn_fc], dim=-1), encoder_hidden_state
        )
        incident_hn_fc = self.fc_incident(incident_hidden_state)
        incident_hn_fc = self.activation(incident_hn_fc)
