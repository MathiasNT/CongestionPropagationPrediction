...
from util_folder.ml_utils.loss_utils import UpstreamBCELoss, UpstreamFocalLoss, FocalLoss
...
class BaseModelClass(pl.LightningModule):
    ...
    def __init__(self, config, learning_rate, pos_weights=None):
        ...
        if self.loss_type == "upstream_focal_loss":
            self.upstream_bce_loss_func = UpstreamFocalLoss(
                pos_weights=pos_weights, gamma=torch.tensor([2])
            )  # TODO Give gamma as feature
    ...
    def standard_step(self, batch, step_type):
        ...
        y_hat = self.forward(inputs=x, incident_info=incident_info, network_info=network_info)
        ...
        if self.loss_type == "bce":
            ...
            upstream_bce_loss = self.upstream_bce_loss_func(
                y_hat[..., 0], y_true[..., 0], batch["network_info"][..., 0]
            )
    ...
    def calc_metrics(self, y_hat, y, step_type):
        ...
        metric_dict[f"{step_type}/prec"], metric_dict[f"{step_type}/rec"] = precision_recall(
            torch.sigmoid(y_hat[..., 0]), y[..., 0].int()
        )
    ...
    def reshape_targets(self, y_hat, y_true, batch_incident_mask, batch_size):
        ...
        if self.form == "incident_only":
            ...
            pred_effect_mask = torch.ge(
                y_hat[..., 0], 0
            )  # BCElosswithlogits use sigmoid inside so >= 0 means positive class
