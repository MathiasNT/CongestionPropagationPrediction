...
class NRI_v1(BaseModelClass):
    def __init__(self, config, learning_rate, pos_weights):
        ...
        encoder_n_in = config['timeseries_in_size'] * config['n_timesteps'] + config['info_in_size'] + config['network_in_size']
        self.encoder = MLPEncoder(n_in=encoder_n_in,
                                 n_hid=config['nri_n_hid'],
                                 n_out=self.n_edge_types,
                                 do_prob=config['dropout'] ,
                                 factor=True,
                                 use_bn=True)
        self.decoder = GRUDecoder(n_hid=config['nri_n_hid'],
                                  ...
                                  skip_first=True,
                                  do_prob=config['dropout'],)
        self.mlp_timeseries = nn.Sequential(nn.Linear(in_features=config['rnn_hidden_size'],
                                                      out_features=config['fc_hidden_size']),
                                            self.activation,
                                            nn.Linear(in_features=config['fc_hidden_size'],
                                                      out_features=config['rnn_hidden_size']),)
        self.mlp_net_info = nn.Sequential(nn.Linear(in_features=config['network_in_size'] + config['info_in_size'],
                                                    out_features=config['fc_hidden_size']),
                                          self.activation,
                                          nn.Linear(in_features=config['fc_hidden_size'],
                                                    out_features=config['rnn_hidden_size']),)
        ...
        self.mlp_shared = nn.Sequential(nn.Linear(in_features=config['fc_hidden_size'],
                                                  out_features=config['fc_hidden_size']),
                                        self.activation,
                                        nn.Linear(in_features=config['fc_hidden_size'],
                                                  out_features=config['fc_hidden_size']))
    ...
    def forward(self, inputs, incident_info, network_info):
        ...
        edges = F.gumbel_softmax(edge_logits, tau=0.5, hard=True) # TODO fix the gumbel_hard to do the right for different step types
    ...
    def standard_step(self, batch, step_type):
        ...
        y_hat, edge_probs = self.forward(inputs=x,
...
class NRI_uninformed(BaseModelClass):
    def __init__(self, config, learning_rate, pos_weights):
        ...
        self.mlp_net_info = nn.Sequential( 
                                          nn.Linear(in_features=config['network_in_size'],
                                                    out_features=config['fc_hidden_size']),
                                          self.activation,
                                          nn.Linear(in_features=config['fc_hidden_size'],
                                                    out_features=config['rnn_hidden_size']),
...
class NRI_uninformed_rw(BaseModelClass):
    def __init__(self, config, learning_rate, pos_weights):
        ...
        self.incident_mpnn1 = MPNN(n_in=config['rnn_hidden_size'],
                                  n_hid=config['nri_n_hid'],
                                  n_out=config['nri_n_hid'],
                                  msg_hid=config['nri_n_hid'],
                                  msg_out=config['nri_n_hid'],
    ...
    def forward(self, inputs, incident_info, network_info):
        ...
        incident_hidden_state = self.incident_mpnn1(timeseries_hn_fc,
                                                   self.rel_rec,
                                                   self.rel_send,
                                                   edges)
