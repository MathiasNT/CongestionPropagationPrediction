...
from models.baselines.lstm_attention import (
    AttentionRNNModel,
    InformedAttentionRNNModel,
    NetworkInformedAttentionRNNModel,
)
from models.MPNN_models.mpnn_model import InformedMPNNModel
from models.MPNN_models.nri_model import NRI_v1, NRI_uninformed, NRI_uninformed_rw
...
def init_model(config, pos_weights):
    ...
    if config["model"] == "attention":
        ...
        model = SimpleGNN(
            adj_mx=adj_mx, args=gnn_args, config=config, learning_rate=config["learning_rate"], pos_weights=pos_weights
        )
        ...
        gnn_args = create_gnn_args(config)
        adj_mx = torch.Tensor(np.load(config["AD_path"]))
        model = InformedGNN_v4(
            adj_mx=adj_mx, args=gnn_args, config=config, learning_rate=config["learning_rate"], pos_weights=pos_weights
        )

    elif config["model"] == "informed_mpnn":
        adj_mx = torch.Tensor(np.load(config["AD_path"]))
        model = InformedMPNNModel(
    ...
    elif config["model"] == "uninformed_nri_rw":  # TODO fix this such that it is nri and informed nri
        model = NRI_uninformed_rw(config=config, learning_rate=config["learning_rate"], pos_weights=pos_weights)
