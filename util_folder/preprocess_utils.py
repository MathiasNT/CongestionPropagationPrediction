...
from .ml_utils.data_utils.network_utils import get_up_and_down_stream
...
def transform_df_to_tensor(df, interpolation_lim=5, warmup=10):
    ...
    Args:
        ...
        interpolation_lim (int, optional): The limit of how many timesteps of missing speeds to interpolate.
                                            Defaults to 5.
    ...
    # Set the new index, resorting the data, padding -1 for missing lanes
    # and then resetting the index s.t. 'interval_begin' is a column
    df = (
        df.set_index(index_names)[["interval_occupancy", "interval_speed", "interval_nVehContrib"]]
        .reindex(multi_index, fill_value=-1)
        .reset_index(level=2)
    )
...
def find_congestion(residual_speed, lane_stds, length_lim, std_lim):
    """Takes in residual speed, lane stds and returns congestions that are longer than 
       length_lim and more severe than std_lim times standard deviation
...
def infer_incident_data(simulation_paths):
    """Takes the detector data and incident information from a SUMO simulation at the given path 
       and creates input and target data for congestion prediction.
    ...
    edge_to_level_dict_num, upstream_edges, _, _ = get_up_and_down_stream(i_edge_obj=i_edge_obj, n_up=100, n_down=100)
    ...
    length_lim = 10
    (affected_edges, _, _) = find_congestion(
        residual_speed=residual_speed, lane_stds=lane_stds, length_lim=length_lim, std_lim=1.95
    )
    ...
    (_, longest_affect_lengths, longest_affect_idxs) = find_congestion(
        residual_speed=residual_speed, lane_stds=lane_stds, length_lim=1, std_lim=0.5
    )
    ...
    incident_info = [
        edge_to_ind[i_edge_obj.getID()],
        len(incident_settings["lanes"]),
        incident_settings["slow_zone_speed"],
        incident_settings["duration_time"],
    ]
