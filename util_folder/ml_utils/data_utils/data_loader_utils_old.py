...
class IncidentDataSet(Dataset):
    ...
    Attributes:
        input_data (torch.Tensor): [N_s, E, L, T_i, F] (Occupancy, Speed, NvehContrib
                                        with traffic data before the incident
...
class IncidentDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        train_set, val_set, test_set = random_split(
            range(len(input_full)), [train_len, test_val_len, test_val_len], generator=torch.Generator().manual_seed(1)
        )
    ...
    def train_dataloader(self):
        train_split = IncidentDataSet(
            self.input_train, self.target_train, self.incident_info_train, self.network_info_train
        )
        return DataLoader(train_split, batch_size=self.batch_size, num_workers=8)

    def val_dataloader(self):
        val_split = IncidentDataSet(self.input_val, self.target_val, self.incident_info_val, self.network_info_val)
        return DataLoader(val_split, batch_size=self.batch_size, num_workers=8)

    def test_dataloader(self):
        test_split = IncidentDataSet(self.input_test, self.target_test, self.incident_info_test, self.network_info_test)
