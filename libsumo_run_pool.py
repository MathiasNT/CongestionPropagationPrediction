...
def get_args():
    ...
    arg_parser.add_argument(
        "--scenario",
        choices=["motorway", "national", "urban", "experiment"],
        help="Which scenatio to run.",
        required=True,
    )
    ...
    arg_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If true only simulates the time around the congestion",
    )
    ...
    arg_parser.add_argument(
        ...
        type=str,
        default="edgedata",
        help="The name of the simulation run. Will be name of numbere results folder.",
    )

    arg_parser.add_argument(
        "--n_random_incidents", type=int, default=0, help="The number of random incidents to simulate"
    )
    arg_parser.add_argument(
        "--n_non_incidents", type=int, default=0, help="The number of simulations without incident to run"
    )
    arg_parser.add_argument(
        "--incidents_settings_file", type=str, default=None, help="Path to the incident settings file"
    )
    ...
    arg_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="For any incident run the counterfactual of no incident",
    )

    arg_parser.add_argument("--trip_info", action="store_true", default=False, help="Save information of all trips.")

    arg_parser.add_argument(
        "--verbose", action="store_true", default=False, help="Save error and message log of SUMO warnings and errors"
    )

    arg_parser.add_argument("--data_frequency", type=int, default=60, help="The time resolution of the output files")
    arg_parser.add_argument(
        "--edge_data_bool", action="store_true", default=False, help="If SUMO edge data output is wanted."
    )
...
def run(simulation_settings, start_time, end_time, incident_settings):
    # TODO this only redirect the print statements. SUMO warnings are a WIP,
    # see https://github.com/eclipse/sumo/issues/10344
    ...
    if simulation_settings["counterfactual"]:
        ...
        print(
            f"Finished counterfactual for {simulation_settings['simulation_folder'].split('/')[-1]} in {end_wtime - start_wtime}"
        )
    else:
...
if __name__ == "__main__":
    ...
    for run_num in range(0, n_runs):
        ...
        if args.do_counterfactuals:
            ...
            jobs.append(
                (
                    counterfactual_sim_settings[run_num],
                    simulation_start_time,
                    simulation_end_time,
                    counterfactual_settings[run_num],
                )
            )
