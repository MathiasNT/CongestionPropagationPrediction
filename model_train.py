...
def run_config(config, overwrite_random_seed, overwrite_gpu):
    ...
    if config["scenario"] == "pems":
        ...
        incident_data_module = RWIncidentDataModule(
        ...
        incident_data_module = IncidentDataModule(
            folder_path=folder_path,
            transform=config["transform"],
            batch_size=config["batch_size"],
            spatial_test=config["spatial_test"],
    ...
    if "checkpoint_path" in config:  # loading a checkpoint
        trainer.fit(model=model, datamodule=incident_data_module, ckpt_path=config["checkpoint_path"])
    else:
        trainer.fit(
            model=model,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--overwrite_random_seed", type=int, help="Overwrite the random seed from the configs", nargs="+"
    )
